...
class TestSyncEntry:
   ...
   def test_create_full_entry(self) -> None:
      ...
      assert entry.local_hash == "sha256-local"
      assert entry.status == SyncStatus.local_modified
...
class TestSyncStateCRUD:
   ...
   async def test_update_existing_entry(self, initialized_state: SyncState) -> None:
      ...
      # Update the entry
      entry.local_hash = "new-hash"
      entry.status = SyncStatus.local_modified
      await initialized_state.set_entry(entry)

      retrieved = await initialized_state.get_entry("update-me.md")
      assert retrieved is not None
      assert retrieved.local_hash == "new-hash"
      assert retrieved.status == SyncStatus.local_modified
   ...
   async def test_list_by_status(self, initialized_state: SyncState) -> None:
      ...
      statuses = [SyncStatus.clean, SyncStatus.local_modified, SyncStatus.conflict]
      ...
      modified = await initialized_state.list_by_status(SyncStatus.local_modified)
   ...
   async def test_count_by_status(self, initialized_state: SyncState) -> None:
      ...
      assert await initialized_state.count_by_status(SyncStatus.local_modified) == 1
...
class TestStatusTransitions:
   ...
   async def test_update_status(self, initialized_state: SyncState) -> None:
      ...
      assert updated.local_hash == "hash"

   async def test_update_hashes(self, initialized_state: SyncState) -> None:
      """Update hash fields independently."""
      entry = SyncEntry(
         path="hash-test.md",
         notion_id="ht-123",
      ...
      # Update only local hash
      await initialized_state.update_hashes("hash-test.md", local_hash="new-local")
      updated = await initialized_state.get_entry("hash-test.md")
      assert updated is not None
      assert updated.local_hash == "new-local"
      assert updated.remote_hash == "old-remote"

      # Update only remote hash
      await initialized_state.update_hashes("hash-test.md", remote_hash="new-remote")
      updated = await initialized_state.get_entry("hash-test.md")
      assert updated is not None
      assert updated.remote_hash == "new-remote"

      # Update both
      await initialized_state.update_hashes(
         "hash-test.md", local_hash="both-local", remote_hash="both-remote"
      )
      updated = await initialized_state.get_entry("hash-test.md")
      assert updated is not None
      assert updated.local_hash == "both-local"
      assert updated.remote_hash == "both-remote"

   async def test_status_transition_clean_to_modified(
      self, initialized_state: SyncState
   ) -> None:
      """Transition from clean to local_modified."""
      entry = SyncEntry(
         path="transition.md",
         notion_id="tr-123",
      ...
      # Simulate local edit
      await initialized_state.update_status("transition.md", SyncStatus.local_modified)
      await initialized_state.update_hashes("transition.md", local_hash="new-hash")

      updated = await initialized_state.get_entry("transition.md")
      assert updated is not None
      assert updated.status == SyncStatus.local_modified
      assert updated.local_hash == "new-hash"
      assert updated.remote_hash == "hash"  # Remote unchanged
...
class TestDatetimeSerialization:
   ...
   async def test_none_datetime(self, initialized_state: SyncState) -> None:
      ...
      assert retrieved is not None
      assert retrieved.local_mtime is None
