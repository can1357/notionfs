...
class TestPull:
   ...
   async def test_pull_updates_changed_file(
      ...
      entry = SyncEntry(
         ...
         is_directory=False,
         local_hash=_compute_hash("Old content"),
         remote_hash=_compute_hash("Old content"),
         local_mtime=old_time,
   ...
   async def test_pull_handles_remote_deletion(
      ...
      entry = SyncEntry(
         ...
         is_directory=False,
         local_hash=_compute_hash("Content"),
...
class TestStatus:
   ...
   async def test_status_returns_pending_changes(
      ...
      statuses = [
         ...
         ("modified.md", SyncStatus.local_modified, "old-hash"),
   ...
   async def test_status_detects_local_modifications(
      ...
      """Status refreshes and detects local file changes."""
      old_hash = _compute_hash("Original")
      entry = SyncEntry(
         path="tracked.md",
         notion_id="tr-123",
      ...
      assert pending[0].status == SyncStatus.local_modified
   ...
   async def test_status_detects_deleted_local(
      ...
      """Status detects locally deleted files."""
      entry = SyncEntry(
         path="deleted.md",
         notion_id="del-123",
